...
There is **no middleware/proxy**, and the service-role path bypasses RLS entirely, so authorization is primarily a code convention. RLS is now a real second layer (see 3a) but must never be the only one:
...
3a. **RLS policies are recorded in `supabase/rls-policies.sql`** (a record, not a migration — the project isn't migration-managed). Every admin-facing policy checks `public.is_admin()`, which compares the JWT `email` claim against a hardcoded allowlist — **keep that list in sync with `ADMIN_EMAILS`**. Two rules when adding a table: (a) never write `FOR ALL TO public USING (true)` — `public` includes `anon`, so that publishes the table to anyone holding the anon key (this exact bug exposed every `valuation_requests` row until 2026-08-29); the service role bypasses RLS and needs no policy. (b) Prefer reading admin data through a `requireAdmin`-gated server action over the anon browser client — `/admin/submissions` and `/admin/valuations` still use the anon client and so depend on these policies being right.
...
5. **Storage uploads go browser → Supabase directly using the logged-in admin's session JWT.** Authorization for those depends on the **`storage.objects` RLS policies** (recorded at the bottom of `supabase/rls-policies.sql`): INSERT and DELETE on `property-images` are `TO authenticated` + `public.is_admin()`, SELECT is public. Until 2026-09-16 these were `TO public`, so anyone with the anon key could upload to or wipe the bucket — check `pg_policies` for `storage.objects` if you ever recreate the bucket. This is an infra dependency *not* enforced in app code.
